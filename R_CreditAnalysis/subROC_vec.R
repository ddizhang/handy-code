...
#'plot ROC curve on subsets of a dataset given a prediction vector and actual value vector, and a subseting variable
#'
...
#'@example # subROC.vec(pb, subsetVar = "home_ownership", binevent = "event.bad", dat = te)
...
subROC.vec = function(prediction, subsetVar, binevent, dat)
{
  library(pROC)
  ...
  for(i in 1:ln)
  {
    ...
    pred = prediction[dat[, subsetVar] == levels(dat[, subsetVar])[l_nonemp[i]]]
